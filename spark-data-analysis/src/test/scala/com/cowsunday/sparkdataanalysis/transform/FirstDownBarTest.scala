...
class FirstDownBarTest extends Specification {

  private val master = "local[2]"
  private val appName = "FirstDownBarTest-test"
  ...
  @Test def testTransform() {
    val priceBars = Seq(new PriceBar(2,2,0.5,3,20150101),
        new PriceBar(2,4,1.5,2.5,20150102),
        new PriceBar(5,6,2.5,4,20150103),
        new PriceBar(14,7,3.5,10,20150104),
        new PriceBar(5,8,4.5,5.5,20150105),
        new PriceBar(1,9,5.5,13,20150106),
        new PriceBar(1,10,6.5,9,20150107))
    val rdd = sc.parallelize(priceBars)

    val results = FirstDownBar.transform(rdd, 3).take(5)
