...
class OpenCloseToHighLowRangeRatioTest extends Assertions with Matchers {

  private val master = "local[2]"
  private val appName = "OpenCloseToHighLowRangeRatio-test"
  ...
  @Test def testTransform() {
    ...
    val results = OpenCloseToHighLowRangeRatio.transform(rdd).take(5)
    
    results(0) should be (-2.0/3 +- 0.0001)
    results(1) should be (-0.6 +- 0.0001)
    results(2) should be (2.0/7 +- 0.0001)
    results(3) should be (6/3.5 +- 0.0001)
    results(4) should be (-.5/3.5 +- 0.0001)
