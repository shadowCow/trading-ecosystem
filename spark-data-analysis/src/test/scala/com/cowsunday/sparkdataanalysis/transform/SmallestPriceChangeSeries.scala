...
class SmallestPriceChangeSeriesTest extends Assertions with Matchers {

  private val master = "local[2]"
  private val appName = "SmallestPriceChangeSeries-test"
  ...
  @Test def testTransform() {
    val priceBars = Seq(new PriceBar(4,2,0.5,3,20150101),  
        ...
        new PriceBar(13,10,6.5,1,20150107))
    val rdd = sc.parallelize(priceBars)
    
    val results = SmallestPriceChangeSeries.transform(rdd, 4, 2).take(3)
